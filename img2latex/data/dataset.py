...
class Im2LatexDataset(Dataset):
    ...
    def __init__(
        ...
        # Initialize caches
        self._formula_cache = {}  # Cache for formulas
            ...
            if len(parts) == 2:
                ...
                if formula_idx >= len(formulas):
                    logger.warning(
                        ...
                        f"There are only {len(formulas)} formulas."
                    )
                    continue
    ...
    def _load_formula(self, idx: int) -> str:
        ...
        Returns:
            Formula string
        """
        # Check if the formula is in the cache
        if idx in self._formula_cache:
            return self._formula_cache[idx]
        ...
        with open(self.formulas_file, "r", encoding="utf-8") as f:
            for i, line in enumerate(f):
                if i == idx:
                    formula = line.strip()
                    # Add to cache
                    self._formula_cache[idx] = formula
                    return formula
        
        # If we get here, the formula was not found
        raise ValueError(f"Formula index {idx} not found in file {self.formulas_file}")
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if self.transform is not None:
            ...
            if self.channels == 1:
                image_tensor = torch.from_numpy(np.array(image)).float().unsqueeze(0) / 255.0
            else:
...
def create_data_loaders(
    ...
    batch_size: int = 128,
    num_workers: int = 4,
    cnn_img_size: Tuple[int, int] = (50, 200),
    resnet_img_size: Tuple[int, int] = (224, 224),
