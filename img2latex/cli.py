...
def train(
    ...
    with console.status("[bold green]Setting up training...", spinner="dots"):
        ...
        tokenizer = LaTeXTokenizer(
            max_sequence_length=config["data"]["max_seq_length"]
        )
        
        # Fit tokenizer on formulas file
        formulas_path = os.path.join(
            config["data"]["data_dir"], 
            config["data"]["formulas_file"]
        )
    ...
    # Train the model
    console.print(f"[bold green]Starting training for experiment '{experiment_name}'[/bold green]")
    try:
        best_metrics = trainer.train()
        console.print(f"[bold green]Training completed successfully![/bold green]")
        console.print(f"Best validation metrics:")
...
def predict(
    ...
    beam_size: int = typer.Option(
        0, 
        help="Beam size for beam search (0 for greedy search)"
    ),
...
def evaluate(
    ...
    # Evaluate
    console.print(f"[bold green]Evaluating on {split} split with {len(data_loader.dataset)} samples[/bold green]")
