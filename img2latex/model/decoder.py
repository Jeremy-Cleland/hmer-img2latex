...
class LSTMDecoder(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"Initialized LSTM decoder with vocab size: {vocab_size}, hidden dim: {hidden_dim}")
    ...
    def forward(
        ...
        embedded = self.embedding(target_sequence)  # (batch_size, seq_length, embedding_dim)
        
        if not self.use_attention:
            # Repeat encoder output for each time step
            # Shape: (batch_size, seq_length, embedding_dim)
            encoder_output_repeated = encoder_output.unsqueeze(1).repeat(1, seq_length, 1)
            ...
            for t in range(seq_length):
                ...
                current_input = embedded[:, t, :].unsqueeze(1)  # (batch_size, 1, embedding_dim)
    ...
    def decode_step(
        ...
        if not self.use_attention:
            ...
            encoder_output_repeated = encoder_output.unsqueeze(1)  # (batch_size, 1, embedding_dim)
            lstm_input = torch.cat([embedded, encoder_output_repeated], dim=2)
