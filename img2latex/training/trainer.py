...
from img2latex.data.dataset import Im2LatexDataset
...
from img2latex.utils.mps_utils import empty_cache, set_device, synchronize
...
class Trainer:
    ...
    def validate(self) -> Dict[str, float]:
        ...
        logger.info(
            ...
            + (f", BLEU: {metrics.get('val_bleu', 0):.4f}" if "val_bleu" in metrics else "")
            + (f", Levenshtein: {metrics.get('val_levenshtein', 0):.4f}" if "val_levenshtein" in metrics else "")
        )
    ...
    def train(self) -> Dict[str, float]:
        ...
        # Update experiment status
        experiment_registry.update_experiment_status(
            self.experiment_name, "completed"
        )
